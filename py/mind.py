...
def _cython_prepare_sequences(data: cython.double[:, :], sequence_length: cython.int):
    n = data.shape[0]
    seq_size = n - sequence_length
    sequences = np.empty((seq_size, sequence_length, 2), dtype=np.float64)
    labels = np.empty((seq_size, 2), dtype=np.float64)
    
    for i in range(seq_size):
        sequences[i] = data[i:i+sequence_length]
        labels[i] = data[i+sequence_length]
        
    return sequences, labels

@cython.boundscheck(False)
@cython.wraparound(False)
def _cython_transform_input(arr: cython.double[:, :], scaler_data_min, scaler_data_max, feature_range):
    low, high = feature_range
    data_range = scaler_data_max - scaler_data_min
    scale = (high - low) / data_range
    min_ = low - scaler_data_min * scale
    return (arr - scaler_data_min) * scale + min_
...
class Mind:
    def __init__(self, sequence_length=20, device=None, download_on_init=False, upload_on_fail=False):
        ...
        if download_on_init and upload_on_fail and not self.model_loaded_successfully:
            logger.warning("Failed to awaken Mind from Cloudinary. Uploading new, untrained Mind as a baseline.")
            self.sleep()

    def _prepare_sequences(self, data):
        # Use Cython optimized function if we have a numpy array
        if isinstance(data, np.ndarray):
            return _cython_prepare_sequences(data, self.sequence_length)
        
        # Fallback to Python implementation for list input
        sequences, labels = [], []
    ...
    def learn(self, data, epochs=50, lr=0.001, batch_size=32):
        ...
        logger.info(f"Mind is learning from {len(data)} data points for {epochs} epochs...")
        self.model.train()
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % 10 == 0 or epoch == epochs - 1:
                logger.info(f"  Epoch [{epoch + 1}/{epochs}], Loss: {epoch_avg_loss:.6f}")
    ...
    def sleep(self, max_attempts=3):
        ...
        logger.info("Mind is going to sleep. Saving memory to the cloud...")
        state = {
        ...
        for attempt in range(max_attempts):
            try:
                ...
                logger.info("✅ Mind's memory successfully stored in the cloud.")
                return True
            except Exception as e:
                logger.warning(f"Cloudinary upload attempt {attempt + 1}/{max_attempts} failed: {e}")
                if attempt < max_attempts - 1:
                    time.sleep(2 ** (attempt + 1))
        logger.error("❌ All attempts to upload Mind's memory failed.")
        return False

    def awaken(self, max_attempts=3):
        """Loads the Mind's state (memory) from Cloudinary."""
        logger.info("Mind is awakening. Loading memory from the cloud...")
        for attempt in range(max_attempts):
            try:
                ...
                # Reconstruct scaler
                self.scaler = MinMaxScaler(feature_range=state.get('scaler_feature_range', (-1, 1)))
                self.scaler.data_min_ = np.array(state['scaler_data_min'])
                self.scaler.data_max_ = np.array(state['scaler_data_max'])
                ...
                logger.info("✅ Mind has awakened successfully.")
                return True
            except Exception as e:
                logger.warning(f"Cloudinary download attempt {attempt + 1}/{max_attempts} failed: {e}")
                if attempt < max_attempts - 1:
                    time.sleep(2 ** (attempt + 1))
        logger.error("❌ All attempts to awaken the Mind failed.")
        self.model_loaded_successfully = False
        return False
...
if __name__ == '__main__':
    ...
    if not mind.model_loaded_successfully:
        logger.info("No pre-trained Mind found. Training a new one.")
        ...
        logger.info("Pre-trained Mind awakened. Ready for predictions.")
